...
def main():
    ...
    def client():
        ...
        for i in [1, 2]:
            ...
            yield from _send_and_wait(ProtocolMessage.create_single(ProtocolMessageType.LOGIN,
                                                        {"username": "testuser{}".format(i)}))

            yield from _send_and_wait(ProtocolMessage.create_single(ProtocolMessageType.CREATE_GAME,
                                            {"board_size": 5,
            ...
            yield from _send_and_wait(ProtocolMessage.create_single(ProtocolMessageType.CHAT_SEND, {"username": "testuser", "text": "bummsfallerafalleri hurz"}))

            yield from _send_and_wait(ProtocolMessage.create_single(ProtocolMessageType.MOVE, {"turn_counter": 2, "ship_id": 146579, "direction": Orientation.EAST}))

            yield from _send_and_wait(ProtocolMessage.create_single(ProtocolMessageType.GAME, {
                "game_id": 60000, "username": "you", "board_size": 7, "num_ships": NumShips([1, 2, 3, 4, 5]), "round_time": 25, "options": GameOptions.PASSWORD}))

            yield from _send_and_wait(ProtocolMessage.create_repeating(ProtocolMessageType.GAMES, [
            ...
            yield from _send_and_wait(ProtocolMessage.create_single(ProtocolMessageType.JOIN, {"game_id": 60000, "password": "bumms"}))
            yield from _send_and_wait(ProtocolMessage.create_single(ProtocolMessageType.JOIN, {"game_id": 60001}))

            yield from _send_and_wait(ProtocolMessage.create_single(ProtocolMessageType.LOGOUT))
